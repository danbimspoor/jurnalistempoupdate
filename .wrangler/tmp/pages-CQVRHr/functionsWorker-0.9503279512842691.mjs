...
var onRequest = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    const article = await context2.env.DB.prepare(`
    ...
    context2.waitUntil(
      context2.env.DB.prepare("UPDATE articles SET views = views + 1 WHERE id = ?").bind(article.id).run()
    );
...
var onRequest2 = /* @__PURE__ */ __name(async (context2) => {
  const { searchParams } = new URL(context2.request.url);
  ...
  params.push(limit);
  try {
    const { results } = await context2.env.DB.prepare(query).bind(...params).all();
...
var onRequest3 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    await context2.env.BUCKET.put(key, file.stream(), {
...
// robots.txt.ts
var onRequest4 = /* @__PURE__ */ __name(async () => {
...
var onRequest6 = /* @__PURE__ */ __name(async (context2) => {
  try {
    const { results } = await context2.env.DB.prepare("SELECT id, slug, published_at FROM articles ORDER BY published_at DESC").all();
...
// ../.wrangler/tmp/pages-CQVRHr/functionsRoutes-0.04139136559772294.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-a97Cp4/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-a97Cp4/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9503279512842691.mjs.map
